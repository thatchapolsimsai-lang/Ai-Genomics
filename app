...
def extract_text(response) -> str:
    """Return only text blocks, excluding Gemini metadata/signatures."""
...
def get_active_llm(model_choice: str, api_key: str):
    """Build the selected LangChain chat model without exposing credentials."""
...
def fetch_online_open_access_context(query: str):
    """Retrieve current biology literature from online Open Access APIs only."""
    context = []
    sources = []
...
def render_online_research():
    ...
    attached_files = st.file_uploader(
        "แนบเอกสารหรือรูปภาพสำหรับคำถามนี้",
        type=["pdf", "txt", "md", "png", "jpg", "jpeg", "webp"],
        accept_multiple_files=True,
        key="online_attachments",
    )
    camera_image = st.camera_input("ถ่ายภาพสำหรับคำถามนี้", key="online_camera")
...
def parse_analysis_json(raw_response: str):
    """Validate the model contract before rendering the dashboard."""
...
def fetch_ncbi_sequence(accession: str):
    """Fetch one GenBank FASTA record and normalize it for the shared pipeline."""
...
def resolve_database_query(sequence_id: str, user_query: str, metrics: dict) -> str:
    """Choose a database search target without requiring a separate user field."""
...
def fetch_ncbi_identification(sequence_id: str, sequence: str):
    """Fetch identifier metadata with NCBI E-utilities; never infer missing fields."""
    result = {
        "query": sequence_id,
...
def fetch_literature_data(query: str):
    """Return only Europe PMC Open Access records for the literature context block."""
...
def build_attachment_content(uploaded_files, camera_image):
    """Convert text/PDF attachments to text and images to multimodal parts."""
    content = []
    attachment_names = []
...
def analyze_bio_context_with_attachments(metrics, attachments, query):
    """Run one-click follow-up analysis using existing Bioinformatics output."""
